/************************************************************************************
 *	6/26/2013, DFZ: this program is heavily modified for the PNNL compression project
 *
 *	Author:	DFZ
 *	History:
 *		6/27/2013: tested on /dev/shm/testfile
 ************************************************************************************
 */

/*
  DFZ: original header comments:
...
/*forward declaration of compression algorithms*/
...
int cache_write(
		...
		char		*fname,
		int			n_ref);
int cache_read(char *fname, uint32_t **data, size_t *f_size);
...
//    retstat = pread(fi->fh, buf, size, offset); /*DFZ: oringal code */

    /*DFZ: changes starts here*/

    /*DFZ: try reading from cache*/
    char fpath[PATH_MAX];
    bb_fullpath(fpath, path);
	uint32_t *data_reload;
	size_t f_size;
	int stat = cache_read((char *)fpath, &data_reload, &f_size);
	assert(!stat);

	/*DFZ: decompress*/
	if (0) /*test for file read*/
//	if (data_reload) /*found in cache*/
	{
//		log_msg("\n DEBUG73 %s-%d: found in cache, f_size = %d. \n",
//				__FILE__, __LINE__, f_size);

		uint32_t *decompressedData = (uint32_t *)malloc(size * sizeof(char));

	    /*print the first two nint32 of the compressed data in file*/
//	    log_msg("data_reload[0] = %x, data_reload[1] = %x or %f \n",
//	    		data_reload[0], data_reload[1], *((float *)data_reload + 1));

		int ret = decode_multiref(data_reload, NLON * NLAYER * sizeof(float),
					decompressedData, NLON * NLAYER * sizeof(float),
					NLAYER, NREF,
					0, NLON * NLAYER); /*change these 2 numbers for partial reading*/

//	    log_msg("buf[0] = %x, buf[1] = %x or %f, \n",
//	    		*(uint32_t *)buf, *((uint32_t *)data_reload + 1), *((float *)data_reload + 1));

//	    int i;
//	    for (i = 0; i < 20; i++)
//	    {
//	    	log_msg("DEBUG73 %s-%d: decompressedData[%d] = %f \n", __FILE__, __LINE__, i, *((float *)decompressedData + i));
//	    }

//		log_msg("ret = %d \n", ret);

		assert(ret); /*shouldn't be empty file*/

		memcpy(buf, decompressedData, sizeof(float) * NLON * NLAYER);

		/*This is very important:
		 * 	retstat should be the expected number of bytes that have been read.*/
		retstat = ret;
	}
	else /*read from file*/
	{
		log_msg("\n DEBUG73 %s-%d: NOT found in cache \n",
				__FILE__, __LINE__);

		uint32_t *compressedData = (uint32_t *)malloc(size * sizeof(char));
		uint32_t *decompressedData = (uint32_t *)malloc(size * sizeof(char));
		retstat = pread(fi->fh, compressedData, size, offset);
		...
		decode_multiref(compressedData, NLON * NLAYER * sizeof(float),
							decompressedData, NLON * NLAYER * sizeof(float),
							NLAYER, NREF,
							0, NLON * NLAYER); /*change these 2 numbers for partial reading*/
		...
		oh_compute += t2 - t1;
		log_msg("d722 dfcompression time: t1 = %f, t2 = %f, diff = %f, oh_compute = %f, \n",
				t1, t2, t2 - t1, oh_compute);

		memcpy(buf, decompressedData, sizeof(float) * NLON * NLAYER);
	}

//	log_msg("retstat = %d, size = %d \n", retstat, size);

	/*DFZ: changes stop here*/

    if (retstat < 0)
    	retstat = bb_error("bb_read read");

    /*DFZ test ENTRY*/
//    char *key_str = "testkey";
//    e.key = key_str;
//    int ret = hsearch(e, FIND);
//    log_msg("DEBUG73 %s-%d: e.key = %s, e.val = %s, ret = %x, \n",
//    		__FILE__, __LINE__, e.key, (char *)e.data, ret);
	     ...
	     struct fuse_file_info *fi)
{
	...
	int ret = size;
	/*don't bother to compress for multiple blocks*/
//	if (0)
	if (! offset)
	{


		//    retstat = pwrite(fi->fh, buf, size, offset); /*DFZ: original code*/

		/*DFZ: changes start here*/

		/*DFZ: compress data*/
		float *compressedData = (float *) malloc(sizeof(float) * NLAT * NLON * NLAYER);
		memset(compressedData, 0, sizeof(float) * NLAT * NLON * NLAYER);

		//    log_msg("d717 %s-%d: buf[16384] = %f, \n",
		//    		__FILE__, __LINE__, *((float*)buf + 16384));


		ret = encode_multiref((float *)buf, NLON * NLAYER * sizeof(float),
					compressedData,	NLON * NLAYER * sizeof(float), NLAYER, NREF);


		log_msg("%s-%d: compressed size = %d bytes, \n",
	...
	/*store the overhead time for compression*/
//	char fname_log[256] = "/home/dzhao/persistent/rootdir/";
//	FILE *ptr;
//	char myip[32] = {0};
//	char *command = "/dev/shm/torusIP.sh";
//	if (NULL != (ptr = popen(command, "r"))) {
//		fgets(myip, 256, ptr);
//	}
//	log_msg("myip = %s, \n", myip);
//	strcat(fname_log, myip);
//	strcat(fname_log, "/log");
//	log_msg("fname_log = %s, \n", fname_log);
//	FILE *fp_log = fopen(fname_log, "a");
//	if (!fp_log)
//		log_msg("failed to open file %s, \n", fname_log);
//	fprintf(fp_log, "Overhead = %lf, Ratio = %lf",
//			oh_compute, (double)ret / (NLON * NLAYER * sizeof(float)));
//	fclose(fp_log);

    retstat = pwrite(fi->fh, (const char *)cache_head, ret, offset);

    /*DFZ: check the global cache*/
//    int i;
//    for (i = 0; i < 20; i++)
//    {
//    	log_msg("INFO %s-%d: cache_head[%d] = %x \n", __FILE__, __LINE__, i, cache_head[i]);
//    }

    /*DFZ: changes end here*/
...
//    e.data = (void *)key_val;
//    ret = hsearch(e, ENTER);
//    log_msg("DEBUG73 %s-%d: e.key = %s, ret = %x, \n",
//    		__FILE__, __LINE__, e.key, ret);
